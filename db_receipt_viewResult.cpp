...
void PODBC::db_receipt_viewResult() {
	...
	SQLBindCol(hstmt, 2, SQL_C_SHORT, &pos_num, sizeof(pos_num), &psnum);

	SQLBindCol(hstmt, 3, SQL_C_SHORT, &receipt_num, sizeof(receipt_num), &rn);
	...
	SQLBindCol(hstmt, 6, SQL_C_SHORT, &unit_price, sizeof(unit_price), &up);

	SQLBindCol(hstmt, 7, SQL_C_SHORT, &amount, sizeof(amount), &at);

	SQLBindCol(hstmt, 8, SQL_C_SHORT, &price, sizeof(price), &pr);
